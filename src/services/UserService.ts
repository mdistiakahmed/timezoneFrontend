...
const UserServiceFunction = (dispatch: any) => {
    ...
    const getAllUsers = async (
        ...
        return meAxios
            .get(ApiEndpoints.user.getUsers, {
    ...
    const createUser = async (data: UserDTO): Promise<any> => {
        return meAxios
            .post(ApiEndpoints.user.createUser, data)
            .then((res) => Promise.resolve(res.data));
    };

    const deleteUser = async (username: string): Promise<any> => {
        return meAxios
            .delete(ApiEndpoints.user.deleteUser(username))
            .then((res) => Promise.resolve(res.data));
    };

    // Change param time both front and back-end
    const updateUser = async (data: UserDTO): Promise<any> => {
        return meAxios.put(ApiEndpoints.user.updateUser, data).then((res) => {
